...
upgrade --keep <N>      留几个版本（默认 2：当前 + 前一个）
upgrade --cache <dir>   换个下载缓存目录——**非 root 先下好、再用 sudo 跑时必须带上它**
                        （root 的 TOOL_DIR 是 /root/…，不带路径就在自己的缓存里找不到东西）
...
工具**曾经不知道自己是哪一版**（没有 `__version__`、没有 `--version`），更新机制无从谈起，
所以第一步是把版本号落到代码里：
...
| 版本自洽 | 由 `upgrade` 拿 `--version` 核对（它才知道期望是哪个 tag） | 老版本没有 `--version`，降级成 `--help` 冒烟并警告 | 有 `--version` 但与下载的 tag 不一致（装错包） |
| `systemctl --version` | `rc=0` 且两流至少一个有输出 | `systemctl` 不存在（容器里没有 systemd） | **存在、但输出空或报加载器错** |
| `journalctl --version` | 同上 | 同上 | 同上 |
| `systemctl is-active <unit>` | 输出非空（`active` / `inactive` / `failed` 都算通过） | `systemctl` 不存在 | 输出为空 |
| `lsof -v` / `ss -V` | 任一条能跑（**硬失败优先**：一条坏就是硬失败，不许“换一条试试”盖掉） | 两条都没装（Linux 上有一个就够） | 存在但输出空 / 加载器报错 |
| `mihomo -v` | 能跑出版本 | 没装内核 | 存在但调用坏 |

判据里有两个反直觉的地方，都是在真机上量出来的（不是设想）：

- **`--version` / `-v` 这类要把 stderr 也算作输出**：`lsof -v` 在 Debian 13 上把版本打在 stderr、
  stdout 是空的（rc=0）。只看 stdout 会把一个完全正常的 lsof 判成故障。
- **`is-active` 这类只看 stdout 非空、忽略返回码**：服务没跑时 rc=3 + `inactive`，
  **连不存在的 unit 都是 rc=4 + `inactive`**——返回码在这里是“答案”不是“成败”。
  而那个 bug 是 rc=1 + **空**输出。按返回码判会把正常状态误判成故障。

关键的判别式是最后两列的差别：**“命令不存在”是环境缺件（warn），“命令存在但调用坏/输出空”
才是硬失败（包把子进程环境污染了）**。
...
- **可写**：第 4~10 步一口气做完。
- **不可写**：把**能做的全做掉**——下载 + 校验放到 `<你的 TOOL_DIR>/upgrade/<tag>/` 缓存起来
  ——然后打印那条命令，**带上 `--cache <绝对路径>`**：

      sudo mihomo-cli upgrade v0.2.0 --cache /home/你/.config/mihomo-cli/upgrade

  root 那一次会命中缓存、跳过下载，只做"解包 + 自检 + rename"，已经缩到最小。
  **`--cache` 不是装饰**：root 的 `TOOL_DIR` 是 `/root/…`，不带路径的话它在自己的缓存里什么也
  找不到，会白重下一遍 22 MB——那样这一步就真成嘴炮了。（这是实现时才发现的设计漏洞。）
  实测：命中缓存那一次 **1.6 秒**（对比首次下载 10 秒上下）。
...
所以取数顺序不是"永远先直连"，而是**看请求大小**（顺序是实测定的，不是拍的）：

- **小请求**（API 查 tag、`SHA256SUMS` 六百字节）：直连优先，失败再走本机内核代理。
- **大文件**（22 MB 的资产）：**有本机代理就先走代理**。理由不是"代理更快"，而是直连的失败
  方式太阴：它不报错，只是**涓涓细流**——实测在 NAS 上直连下资产，**3.5 分钟拿到 0 字节**，
  而超时机制对"一直有字节但慢得离谱"完全无效。第一次跑真机 E2E 就卡在这儿。

端口用 `proxy_port()` 读（`MIHOMO_PORT` > `config.yaml` 的 `mixed-port` > 7890）；
本机没东西在听就只剩直连一条路。走哪条**要在输出里说明**，别偷偷换路。
...
- 并发：`TOOL_DIR/upgrade.lock`（`O_EXCL`）挡住两个 upgrade 同时干；拿不到锁就报"另一个升级在跑"。
  锁里记着 PID，**持有者已经不在了就自动接管**（终端一断、进程被 kill，`finally` 跑不到，那种
  死锁不该逼人先去手删一个文件——实测就碰到过）。
- `upgrade --rollback`：把 symlink 指回保留的上一版（含 `legacy-*`），**不联网**，并顺手跑一次
  自检确认切过去的这份是好的。实现在 `update-state.json` 里**不需要记指针**：候选就是"除当前
  之外，mtime 最新的那个入口"，切完之后原来的"当前"自然成了新候选，于是两次 `--rollback`
  就是来回切。
- **但这条只在两份都认 `upgrade` 时成立**：滚回 0.2.0 **之前**的老版本后，那个子命令本身就不
  存在了（老版本没有它），得按路径直接调另一份：

      /usr/local/libexec/mihomo-cli-<新版本>/mihomo-cli upgrade --rollback

  这条提示由 `_rollback()` 在目标不认识 `upgrade` 时主动打出来。**这个洞是真机验收时才发现的**
  ——当时我照着文档里那句"再跑一次 --rollback"敲下去，得到的是 `invalid choice: 'upgrade'`。
...
写代码前先验的几件事现在有结论了，另外把整条链路也在真机上跑了一遍。数据来自两个平台各自的
原生产物（`…-macos-arm64.tar.gz` / `…-linux-x86_64.tar.gz`），不是"理论上应该能跑"。
...
### 4. 真机 E2E：整条链路跑通了（沙箱里拿真的 v0.1.1 当"旧安装"）

在 NAS 上搭一个沙箱前缀，放一份**真的 v0.1.1**（就是那台机器的旧布局），然后让新代码走完整条
`upgrade`：真去 GitHub 下载 → 真校验 → 解包 → 自检 → 迁移 → 原子切换 → 写包装脚本。结果：

| 验的是什么 | 结果 |
|---|---|
| 死锁自动接管 | 先前被杀的进程留下的锁被识别为残留（持有者已不在）并接管 ✓ |
| 大文件走本机代理 | 22 MB 资产走 7890 下来，干净利落 ✓ |
| 老版本自检降级 | v0.1.1 不认识 `--version` / `doctor` → 降级为 `--help` 冒烟并明说 ✓ |
| 迁移 | `libexec/mihomo-cli/`（真目录）→ `mihomo-cli-legacy-20260923` ✓ |
| 切换 + 包装脚本 | symlink 指向 `mihomo-cli-0.1.1`；`bin/mihomo-cli` 隔着 symlink 真能跑起那份二进制 ✓ |
| 缓存命中 | 第二次升级 **1.6 秒**（复用已校验的包），这正是 sudo 那一步依赖的 ✓ |
| 回滚 | 切回 legacy、再切回来（回滚本身可回滚），两次包装脚本都还能跑 ✓ |
| `--check` | 真网络查得 `v0.1.1`；把自报版本假装成 0.1.0 时退出码 **10**；缓存写入了 ETag ✓ |
| `status` 提示 | 缓存里放 `v9.9.9` → 打出那行；放 `v0.0.1` → 不打 ✓ |

最后一项**只在 Linux 上暴露**：`status` 的收尾代码当时在两个平台分支里各写了一份，我给
macOS 那份加了提示、Linux 那份漏了——在 Mac 上怎么测都对。现在收尾收敛成 `tail()` 一处，
并有 `tests/test_status.py` 守着。这条是"真机 E2E 不能省"的最好例证。
