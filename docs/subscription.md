...
| `sub set <链接>`（链接没变） | 只让内核重拉节点 | **不碰**，一个字节都不改 |
...
- **能活过重启**靠骨架里补的那行 `profile: store-selected: true`（内核把选择存进 `cache.db`）。
  实测：切到 3 号 → 重启内核 → 仍然是 3 号。不想要这个持久化就把那行删掉/改成 false，
  或者用 `sub use --auto` 回到自动。
...
```yaml
geo-auto-update: true         # geodata 数据文件（GeoSite.dat）自动更新
geo-update-interval: 24       # 每 24 小时检查一次
geox-url:
  geosite: "https://testingcf.jsdelivr.net/..."   # ← 必须换源，见下
```
...
`sub set` 会被自己的校验挡回来（写完 → 校验失败 → 回滚），规则根本装不进去。换成文档里给的
jsdelivr 镜像后实测 2.9 秒下完、`Finished initial GeoSite rule cn => DIRECT, records: 111021`，
落地 `GeoSite.dat` 4.2 MB（放在内核目录里，跟 `geoip.metadb` 一样由内核自己维护）。

`geoip` / `mmdb` / `asn` 三项的默认源本来就是 jsdelivr，所以只覆盖 `geosite` 这一个键；
`geox-url` 整节如果已经存在（你自己配过源），工具**完全不动它**。
...
**链接没变**：直接进刷新流程，`config.yaml` 一个字节都不改。所以 `sub set` 是幂等的：
脚本里反复跑它不会把用户的配置越改越乱（备份目录里也不会堆一串没人看的备份）。
