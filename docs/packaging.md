...
**上传→下载往返会自校验**：构建 job 在上传前把每个交付文件的 sha256 写进 `hashes/<平台>.sha256`
（跟产物放同一个 artifact），release job 下载后先 `sha256sum --check hashes/*.sha256`，
不过就直接红。为什么要多这一步：`SHA256SUMS` 是**下载之后**才算的，只能证明「下载后的文件
自洽」——万一 artifact 往返有损（截断），它照样自洽。而 `hashes/` 是中间产物，只用来对哈希，
**不发布**（所以 `gh release upload` 是显式列文件，不能用 `assets/*`）。
