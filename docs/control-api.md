...
| `src/mihomo_cli/subs.py:959` / `:858` | `GET /providers/proxies/{名}` | `sub show` 与刷新后的回显：节点数、上次更新时间 |

写接口两个：`src/mihomo_cli/subs.py:999` 的 **`PUT /providers/proxies/{名}`**（`sub update`，
