...
| `src/mihomo_cli/subs.py:960` / `:968` | `GET /providers/proxies/{名}` | `sub show` 与刷新后的回显：节点数、上次更新时间 |

写接口两个：`src/mihomo_cli/subs.py:1000` 的 **`PUT /providers/proxies/{名}`**（`sub update`，
