...
FILES = {
    ...
    "geoip.dat": (
        "GEOIP 规则查的 IP 库（dat，geodata-mode: true 时用）",
        "有 GEOIP 规则 + geodata-mode",
    ),
...
FILE_NAMES = tuple(
    FILES
)  # 给 --help 用（dict 顺序就是列的顺序）          # 上限：最大的 geoip.dat 也就几十 MB，防呆
...
def needed() -> tuple[list[str], str]:
    ...
    try:
        start = next(i for i, line in enumerate(lines) if line.startswith("rules:"))
    except StopIteration:
        return [], "配置里没有 rules: 区块"
    types = {
        line[2:].split(",")[0].strip().upper()
...
def cmd_geodata(args: argparse.Namespace) -> int:
    ...
    return {"list": cmd_geodata_list, "download": cmd_geodata_download, "apply": cmd_geodata_apply}[
        action
    ](args)
...
def cmd_geodata_list(_: argparse.Namespace) -> int:
    ...
    print()
    print(
        f"  {pad('文件', 20)}{pad('实体', 12)}{pad('内核目录', 12)}{pad('大小', 10)}sha256（前 16）"
    )
    for name in FILES:
    ...
    need_ap = [
        n
        for n in want
        if (geodata_dir() / n).exists() and not same_file(_kernel_path(n), geodata_dir() / n)
    ]
    if need_dl:
        print(
            f"{warn('⚠ 还没下载：' + '、'.join(need_dl))}  "
    ...
    if geo_bak:
        print(
            dim(
                f"  备份：{len(geo_bak)} 份旧版数据文件在 {BACKUP_DIR}"
...
def cmd_geodata_download(args: argparse.Namespace) -> int:
    ...
    for name in names:
        ...
        if src.exists() and not args.force:
            print(
                f"  {dim('=')} {pad(name, 20)}已有 {size_str(src.stat().st_size)}"
                f"  {dim('（--force 可重下）')}"
            )
            same += 1
            continue
        ...
        if expect and expect != digest:
            die(
                f"{name}：sha256 和同源的 .sha256sum 不一致，拒绝写入\n"
                f"    期望 {expect}\n    实际 {digest}"
            )
        ...
        note = (
            "（sha256 已对齐同源 .sha256sum）" if expect else "（同源没有 .sha256sum，只算了一遍）"
        )
        print(
            f"  {ok('✓')} {pad(name, 20)}{pad(size_str(len(data)), 10)}"
...
def cmd_geodata_apply(args: argparse.Namespace) -> int:
    ...
    if args.reload:
        ...
        print(
            dim(
                "  数据文件是内核启动时读的；让它生效：mihomo-cli restart（或 geodata apply --reload）"
            )
        )
    return 0
