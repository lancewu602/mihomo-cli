...
#
# 里面**只有两项是内核默认值做不到的**（`geodata-mode` 和 `external-controller`，各自下面
# 写了原因）；其余都是「内核默认值本来就对」或者「只差一个键」的口味项，照样写出来是因为这份
# 骨架是给人读的：`sub set` 完 config.yaml 里能一眼看到本工具依赖哪几项，不用去翻手册。每一项
# 都能自己改——已有的键一律不覆盖，删掉或改掉都行（嵌套节也只补缺的子键）。
# （内核默认值做不到的第三项是 `geox-url`，它是嵌套节，见下面 GEOX_MIRROR 那一段。）
GLOBAL_SCALARS = (
