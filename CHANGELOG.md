...
- **显式命令不再被缓存冒充**。`upgrade` / `upgrade --check` 现在**永远问网络**，缓存只服务
  `status` 那行被动提示（ETag 仍然带上，命中 304 连配额都不耗；真查不通才退回缓存并说明）。
  旧行为会实实在在骗人：v0.2.1 刚发出去，机器上 `--check` 还说“已是最新”——因为缓存里是几分钟前
  查到的 v0.2.0。**刚发完版的那一刻恰恰是最想检查的时刻**，那时候骗人最要命。

### 变更

- **不再出 macOS x86_64（Intel）产物**：CI 矩阵去掉 `macos-15-intel`，`install.ASSET_SUFFIXES`
  同步去掉（两边靠 `tests/test_release_contract.py` 绑着，只改一边会当场失败）。Intel Mac 上
  `upgrade` 会明确说“本平台没有预编译包”并给出 `uv tool install` / 源码两条路，而不是拼一个
  必 404 的名字。
...
- 下载完那一刻多报一行：**大小 / 耗时 / 校验通过**（并把自己那行加个 `✓`）。
  "直连涓流卡死"那种情况下，这行是唯一能看出端倪的东西——它不报错，只是慢。
- 非二进制形态的提示加个引号：`你是「源码 checkout」形态的`（原来"checkout的"挤在一起）。
...
- `upgrade`：**二进制版能自更新**。取包（查最新 tag → 按平台拼资产名 → 下载 → 校验 SHA256）
  + 换包（解到暂存区 → 用**将来要上岗的那份**跑 `doctor` 自检 → symlink 原子切换 → 留当前 + 前一个）。
  `--check` 只看（退出码 0 最新 / 10 有新版 / 1 查不动）、`<tag>` 指定版本（降级、钉住）、
  `--rollback` 无须联网回退、`--sha256` 钉死校验和、`--keep` / `--prefix` / `--cache`。
  pip / uv / 源码 checkout 装的一律**不动手**，只报应该跑的命令。
- `status` 顺带提示「有新版本」（**只读缓存，一个字节的网络请求都不发**）。

### 变更

- **安装布局改成版本化目录 + symlink**（`make install` / `uninstall` 同步）：实体在
  `libexec/mihomo-cli-<版本>/`，`libexec/mihomo-cli` 是指向它的 symlink，`bin` 里是 exec 包装
  脚本。切换用 `os.replace` 换 symlink（原子）——原地覆盖会留下一段“路径恰好不在”的窗口。
  旧布局（非版本化真目录）在首次升级时自动留成 `mihomo-cli-legacy-<日期>` 快照。
